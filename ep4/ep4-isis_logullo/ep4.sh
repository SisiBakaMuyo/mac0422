...
for arq in "$@"; do
    ...
    if ((arqc > 1)); then
        ...
        for servidor in "${lista_servidores[@]}"; do
            ...
            /tmp/./$servidor   
            PID_servidor=$(cd /tmp | pgrep -f $servidor)
            hora_subida_serv=$(date +'%Y-%m-%d %H:%M:%S')
            ...
            while ((output < $1)); do
                output=$(journalctl -q --since "$hora_subida_serv" | grep $servidor | grep "provavelmente enviou um exit" | wc -l)
            done

            echo "Verificando os instantes de tempo no journald..."
            dia_ini=$(journalctl -q --since "$hora_subida_serv" | grep $servidor | grep "Passou pelo accept :-)" | head -n 1 | cut -c 1-6)
            hora_ini=$(journalctl -q --since "$hora_subida_serv" | grep $servidor | grep "Passou pelo accept :-)" | head -n 1 | cut -c 8-15)
            data_ini=$(date -d "$dia_ini" +%Y-%m-%d)
            inicio="$data_ini $hora_ini"
            
            dia_fim=$(journalctl -q --since "$hora_subida_serv" | grep $servidor | grep "provavelmente enviou um exit" | tail -n 1 | cut -c 1-6)
            hora_fim=$(journalctl -q --since "$hora_subida_serv" | grep $servidor | grep "provavelmente enviou um exit" | tail -n 1 | cut -c 8-15)
...
set ylabel 'Tempo para atender $1 clientes concorrentes'
set terminal pdfcairo
set output \"ep4-resultados-$1.pdf\"
set grid
set key top left
...
echo "load \"/tmp/ep4-resultados-$1.gpi\"" | gnuplot
